/*
RestServices

REST APIs used for managing CDC instances are captured in this section.

API version: 1.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package sfssrest

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the HostsGETID type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &HostsGETID{}

// HostsGETID This model contains information about the specified host based on the host ID. 
type HostsGETID struct {
	// Unique Identity of the Entity - Fully Qualified Name (FQN) of the host
	Id string `json:"Id"`
	// NVMe Qualified Name (NQN) of the host
	NQN string `json:"NQN"`
	// IP address of the host
	TransportAddress string `json:"TransportAddress"`
	// IP address family; possible values include IPv4 and IPv6
	TransportAddressFamily string `json:"TransportAddressFamily"`
	// Protocol used for communication with the host; possible value is TCP
	TransportType string `json:"TransportType"`
	// Transport Request Type (TREQ); possible values include Secure channel not specified, Secure channel required, and Secure channel not required
	TREQ string `json:"TREQ"`
	// Possible values are No Security and TLS
	TSAS string `json:"TSAS"`
	// Type of endpoint registration; possible values include Implicit, Explicit, Pull, and Manual
	RegistrationType string `json:"RegistrationType"`
	// Status of the TCP connection between the host and the CDC instance; possible values include Online and Offline
	ConnectionStatus string `json:"ConnectionStatus"`
	// Reason for the host being offline; possible values include NONE, KATO, and Peer Closure
	FailureReason string `json:"FailureReason"`
	OdataId string `json:"@odata.id"`
	OdataType string `json:"@odata.type"`
	OdataContext string `json:"@odata.context"`
}

type _HostsGETID HostsGETID

// NewHostsGETID instantiates a new HostsGETID object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewHostsGETID(id string, nQN string, transportAddress string, transportAddressFamily string, transportType string, tREQ string, tSAS string, registrationType string, connectionStatus string, failureReason string, odataId string, odataType string, odataContext string) *HostsGETID {
	this := HostsGETID{}
	this.Id = id
	this.NQN = nQN
	this.TransportAddress = transportAddress
	this.TransportAddressFamily = transportAddressFamily
	this.TransportType = transportType
	this.TREQ = tREQ
	this.TSAS = tSAS
	this.RegistrationType = registrationType
	this.ConnectionStatus = connectionStatus
	this.FailureReason = failureReason
	this.OdataId = odataId
	this.OdataType = odataType
	this.OdataContext = odataContext
	return &this
}

// NewHostsGETIDWithDefaults instantiates a new HostsGETID object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewHostsGETIDWithDefaults() *HostsGETID {
	this := HostsGETID{}
	return &this
}

// GetId returns the Id field value
func (o *HostsGETID) GetId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Id
}

// GetIdOk returns a tuple with the Id field value
// and a boolean to check if the value has been set.
func (o *HostsGETID) GetIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Id, true
}

// SetId sets field value
func (o *HostsGETID) SetId(v string) {
	o.Id = v
}

// GetNQN returns the NQN field value
func (o *HostsGETID) GetNQN() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.NQN
}

// GetNQNOk returns a tuple with the NQN field value
// and a boolean to check if the value has been set.
func (o *HostsGETID) GetNQNOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.NQN, true
}

// SetNQN sets field value
func (o *HostsGETID) SetNQN(v string) {
	o.NQN = v
}

// GetTransportAddress returns the TransportAddress field value
func (o *HostsGETID) GetTransportAddress() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.TransportAddress
}

// GetTransportAddressOk returns a tuple with the TransportAddress field value
// and a boolean to check if the value has been set.
func (o *HostsGETID) GetTransportAddressOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.TransportAddress, true
}

// SetTransportAddress sets field value
func (o *HostsGETID) SetTransportAddress(v string) {
	o.TransportAddress = v
}

// GetTransportAddressFamily returns the TransportAddressFamily field value
func (o *HostsGETID) GetTransportAddressFamily() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.TransportAddressFamily
}

// GetTransportAddressFamilyOk returns a tuple with the TransportAddressFamily field value
// and a boolean to check if the value has been set.
func (o *HostsGETID) GetTransportAddressFamilyOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.TransportAddressFamily, true
}

// SetTransportAddressFamily sets field value
func (o *HostsGETID) SetTransportAddressFamily(v string) {
	o.TransportAddressFamily = v
}

// GetTransportType returns the TransportType field value
func (o *HostsGETID) GetTransportType() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.TransportType
}

// GetTransportTypeOk returns a tuple with the TransportType field value
// and a boolean to check if the value has been set.
func (o *HostsGETID) GetTransportTypeOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.TransportType, true
}

// SetTransportType sets field value
func (o *HostsGETID) SetTransportType(v string) {
	o.TransportType = v
}

// GetTREQ returns the TREQ field value
func (o *HostsGETID) GetTREQ() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.TREQ
}

// GetTREQOk returns a tuple with the TREQ field value
// and a boolean to check if the value has been set.
func (o *HostsGETID) GetTREQOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.TREQ, true
}

// SetTREQ sets field value
func (o *HostsGETID) SetTREQ(v string) {
	o.TREQ = v
}

// GetTSAS returns the TSAS field value
func (o *HostsGETID) GetTSAS() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.TSAS
}

// GetTSASOk returns a tuple with the TSAS field value
// and a boolean to check if the value has been set.
func (o *HostsGETID) GetTSASOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.TSAS, true
}

// SetTSAS sets field value
func (o *HostsGETID) SetTSAS(v string) {
	o.TSAS = v
}

// GetRegistrationType returns the RegistrationType field value
func (o *HostsGETID) GetRegistrationType() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.RegistrationType
}

// GetRegistrationTypeOk returns a tuple with the RegistrationType field value
// and a boolean to check if the value has been set.
func (o *HostsGETID) GetRegistrationTypeOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.RegistrationType, true
}

// SetRegistrationType sets field value
func (o *HostsGETID) SetRegistrationType(v string) {
	o.RegistrationType = v
}

// GetConnectionStatus returns the ConnectionStatus field value
func (o *HostsGETID) GetConnectionStatus() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.ConnectionStatus
}

// GetConnectionStatusOk returns a tuple with the ConnectionStatus field value
// and a boolean to check if the value has been set.
func (o *HostsGETID) GetConnectionStatusOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ConnectionStatus, true
}

// SetConnectionStatus sets field value
func (o *HostsGETID) SetConnectionStatus(v string) {
	o.ConnectionStatus = v
}

// GetFailureReason returns the FailureReason field value
func (o *HostsGETID) GetFailureReason() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.FailureReason
}

// GetFailureReasonOk returns a tuple with the FailureReason field value
// and a boolean to check if the value has been set.
func (o *HostsGETID) GetFailureReasonOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.FailureReason, true
}

// SetFailureReason sets field value
func (o *HostsGETID) SetFailureReason(v string) {
	o.FailureReason = v
}

// GetOdataId returns the OdataId field value
func (o *HostsGETID) GetOdataId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.OdataId
}

// GetOdataIdOk returns a tuple with the OdataId field value
// and a boolean to check if the value has been set.
func (o *HostsGETID) GetOdataIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.OdataId, true
}

// SetOdataId sets field value
func (o *HostsGETID) SetOdataId(v string) {
	o.OdataId = v
}

// GetOdataType returns the OdataType field value
func (o *HostsGETID) GetOdataType() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.OdataType
}

// GetOdataTypeOk returns a tuple with the OdataType field value
// and a boolean to check if the value has been set.
func (o *HostsGETID) GetOdataTypeOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.OdataType, true
}

// SetOdataType sets field value
func (o *HostsGETID) SetOdataType(v string) {
	o.OdataType = v
}

// GetOdataContext returns the OdataContext field value
func (o *HostsGETID) GetOdataContext() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.OdataContext
}

// GetOdataContextOk returns a tuple with the OdataContext field value
// and a boolean to check if the value has been set.
func (o *HostsGETID) GetOdataContextOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.OdataContext, true
}

// SetOdataContext sets field value
func (o *HostsGETID) SetOdataContext(v string) {
	o.OdataContext = v
}

func (o HostsGETID) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o HostsGETID) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["Id"] = o.Id
	toSerialize["NQN"] = o.NQN
	toSerialize["TransportAddress"] = o.TransportAddress
	toSerialize["TransportAddressFamily"] = o.TransportAddressFamily
	toSerialize["TransportType"] = o.TransportType
	toSerialize["TREQ"] = o.TREQ
	toSerialize["TSAS"] = o.TSAS
	toSerialize["RegistrationType"] = o.RegistrationType
	toSerialize["ConnectionStatus"] = o.ConnectionStatus
	toSerialize["FailureReason"] = o.FailureReason
	toSerialize["@odata.id"] = o.OdataId
	toSerialize["@odata.type"] = o.OdataType
	toSerialize["@odata.context"] = o.OdataContext
	return toSerialize, nil
}

func (o *HostsGETID) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"Id",
		"NQN",
		"TransportAddress",
		"TransportAddressFamily",
		"TransportType",
		"TREQ",
		"TSAS",
		"RegistrationType",
		"ConnectionStatus",
		"FailureReason",
		"@odata.id",
		"@odata.type",
		"@odata.context",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varHostsGETID := _HostsGETID{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varHostsGETID)

	if err != nil {
		return err
	}

	*o = HostsGETID(varHostsGETID)

	return err
}

type NullableHostsGETID struct {
	value *HostsGETID
	isSet bool
}

func (v NullableHostsGETID) Get() *HostsGETID {
	return v.value
}

func (v *NullableHostsGETID) Set(val *HostsGETID) {
	v.value = val
	v.isSet = true
}

func (v NullableHostsGETID) IsSet() bool {
	return v.isSet
}

func (v *NullableHostsGETID) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableHostsGETID(val *HostsGETID) *NullableHostsGETID {
	return &NullableHostsGETID{value: val, isSet: true}
}

func (v NullableHostsGETID) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableHostsGETID) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


